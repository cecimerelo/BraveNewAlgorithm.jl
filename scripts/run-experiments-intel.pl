...
say $fh "work,dimension,population_size,max_gens,alpha,DRAM, Psys, PKG,seconds,generations,diff_fitness,evaluations";
    ...
    for my $max_gens ( qw(10 50) ) {
      for my $alpha ( qw(10 25) ) {
        ...
        do {
          ...
          say $output;
          my ( $ram, $cores, $psys, $gpu, $pkg, $seconds ) = process_pinpoint_output_intel $output;
          if ($gpu != 0 ) {
            $successful++;
