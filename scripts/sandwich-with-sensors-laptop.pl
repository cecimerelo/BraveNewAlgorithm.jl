...
use Utils qw(process_pinpoint_output_intel process_sensors_output_intel);
...
say $fh "work,dimension,population_size,max_gens,alpha,RAM,PKG,seconds,generations,diff_fitness,evaluations,initial_temp";
...
sub run_sensors {
  my $output = `sensors`;
  return process_sensors_output_intel( $output );
}
...
sub run_command_for_preffix {
  ...
  say "\nRunning $pre_preffix";
  my ( $foo, $ram, $gpu, $pkg, $seconds, $output );
  ...
  do {
    ...
    say $output;
    ( $ram, $pkg, $foo, $gpu, $foo, $seconds ) = process_pinpoint_output_intel $output;
  } while ( $gpu == 0 );
  ...
  my @results = ($ram,$pkg,$seconds,$generations, $best_fitness-$target_fitness, $evaluations);
  say join(", ", @results);
