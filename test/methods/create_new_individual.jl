...
@testset "create_new_individual" begin
    ...
    offspring1, offspring2 = create_new_individual(parents, test_mutation_rate, test_range)
    ...
    # Test that offspring are within range
    @test all(gene >= test_range[1] && gene <= test_range[2] for gene in offspring1)
    @test all(gene >= test_range[1] && gene <= test_range[2] for gene in offspring2)
end
